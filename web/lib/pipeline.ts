...
export async function runPipeline(
  ...
  const retractionTargets = citations.filter(
    (citation) => citation.status !== "unverified" && Boolean(citation.doi),
  );
  await mapPool(retractionTargets, POOL_RETRACTION, async (c) => {
    try {
      c.status = "checking";
  ...
  const cascadeTargets = citations.filter(
    (citation) =>
      citation.status !== "retracted" &&
      citation.status !== "unverified" &&
      Boolean(citation.doi),
  );
  await mapPool(cascadeTargets, POOL_CASCADE_FETCH, async (c) => {
    try {
      c.status = "checking";
      ...
      const refChecks = await Promise.all(
        refs.map(async (ref) => ({
          ref,
          retraction: await isRetractedWrapper(ref.doi),
        })),
      );
      const cascadeHit = refChecks.find((result) => result.retraction);
      const cascadeViaRef: ReferenceRow | null = cascadeHit?.ref ?? null;
      const cascaded = cascadeViaRef !== null;
  ...
  const replacementTargets = citations.filter(
    (citation) =>
      citation.status === "retracted" || citation.status === "cascade",
  );
  await mapPool(replacementTargets, POOL_REPLACEMENTS, async (c) => {
    try {
      const replacements = await findReplacementPapersWrapper(c.title, c.year);
      c.replacements = replacements;
      if (convexClient && writeSecret) {
