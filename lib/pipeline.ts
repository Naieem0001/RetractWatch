...
function historicalPayloadForJob(
  comparison: ReturnType<typeof compareToHistoricalCases>,
) {
  return {
    matchedCase: comparison.matchedCase?.name ?? "",
    similarity: String(comparison.similarity),
    avgMonthsToCatch: comparison.avgMonthsToCatch ?? 0,
    impactDescription: comparison.impactDescription,
